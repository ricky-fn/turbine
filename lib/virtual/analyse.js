...
var directives = [{
    ...
    use: function use(params, insertQueue, stop) {
        ...
        if (Object.getOwnPropertyDescriptor(properties, inner)) {
            throw "this val name \"" + inner + "\" has been used.";
        }
        ...
        for (key in evalue) {
            ...
            context[inner] = evalue instanceof Array ? evalue[key] : key;
    ...
    bind: function bind(el, binding, vNode) {
        ...
        var value = (0, _eval2.default)(args.content, context);
        var once = true;

        data[args.inner] = value[args.key];
        var _react = new _observe.react(context, data, null);
        var _watcher = new _watcher3.default(context, args.content, function (oldVal, newVal) {
            if (once) {
                return once = false;
            }
            var key = Object.keys(newVal)[args.key];

            if (!newVal.hasOwnProperty(args.key)) {
                _watcher.unwatch();
                return _react = _watcher = null;
            }

            context[args.inner] = newVal instanceof Array ? newVal[args.key] : key;
        });
    }
}, {
    directive: 'show$',
    ...
    if (reg != null || reg != undefined) {
        reg.forEach(function (match) {
            ...
            vNode.inserted(function (node) {
                new _watcher3.default(context, content, function (oldVal, newVal) {
